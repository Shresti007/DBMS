...
class LinkedList:
    ...
    def print_LL(self):
        if self.head is None:
            ...
            n = self.head         # a to write short term to ease the coding we take first node which is curr_node as n.
            while n is not None:  # we use while loop because as we know the stopping condition.
                print(n.data, "--->", end="")
                n = n.next

    def addBegn(self, data):
        new_Node = Node(data)      # Allocation of node and kept it in given point of data
    ...
    def add_end(self, data):    # time complexity is o(n), space complexity is o(1)
        new_Node = Node(data)
        if self.head is None:  # checking if linked list is empty
            ...
            while n.next is not None:
                n = n.next       # now self.head is last node
            n.next = new_Node    # now last nodes next is newly inserted node

    def insert_afr_node(self,data,x): # time complexity of insertion after is o(1)
